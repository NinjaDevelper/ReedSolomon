...
POLY& ReedSolomon::correct(POLY& msg, int num,vector<int>& pos){
    ...
    //calculate X_j*evaluator(X_j-^1)/locator'(X_j^-1)=evaluator(X_j-^1)/( X_j^-1 * locator'(X_j^-1))
    for(i=0;i<errNum;i++){
        unsigned char minus_i = expTable[pos[i]+256-msg.n];
        unsigned char evaluator_minus_i = evaluatePoly(evaluator, minus_i);
        unsigned char locator_minus_i = evaluatePoly(locator, multiply(evaluator_minus_i,evaluator_minus_i));
        msg.data[pos[i]] ^= divide(evaluator_minus_i, multiply(minus_i,locator_minus_i));
    }
    free(tmp);
    free(synd);
    free(locator.data);
...
int ReedSolomon::decodeFile(string inFile,string outFile,int K,int N,long size,vector<long>&errLoc){
    int i=0;
    int k=0;
    ...
    unsigned char *buf = 
        (unsigned char*)mmap(NULL, inSize, PROT_READ|PROT_WRITE, MAP_PRIVATE, fin, 0);

    int interleave=ceil((double)size/K);
    map<int, vector<int> > errors;

    //translate location in file to column/row number of K x interleave length matrix.
...
int ReedSolomon::encodeFile(string origFile,string encodedFile,int K,int N){
    ...
    unsigned char *outBuf = 
        (unsigned char*)mmap(NULL, writeSize, PROT_WRITE, MAP_SHARED, fout, 0);
